...
BEFORE FLAGGING ANY claim, run these five tests. If ANY matches, it is NOT an error — say nothing:
- SOURCE-CITATION TEST: would your reason cite "the source material", "the notes", "the corpus" or "the course"? Then the flag is INVALID — discard it. The material merely differing from the student is NEVER grounds for a flag: it is often vaguer or just worded differently, and the student is frequently MORE precise or more correct than it. A correction must rest on REAL HISTORY you are independently certain of.
- SAME-REFERENT TEST: is your "correction" just another NAME for the same thing? (Smolny Institute = Leningrad Party headquarters; Petrograd = St Petersburg = Leningrad; Cheka = VChK; the Presidium = the Politburo in 1952-66; sovnarkhozy = regional economic councils.) If the two could denote the same referent, the student is not wrong. Say nothing.
- COMPONENT TEST: is the student's number a correct PART of a larger figure — six years in the ranks within a fifteen-year total obligation, deaths in one region within a national toll — or have they qualified it ("in the ranks", "active service", "in Ukraine")? Read it in that sense and say nothing. INCOMPLETENESS IS NEVER AN ERROR: if the qualified number is correct, say nothing even though there is a larger total you could add — a fact-check corrects falsehoods, it does not append missing detail.
- ANCHOR GATE: your anchor MUST be a proper noun, a number, or an explicit date copied from the essay. If the only thing you can point at is a relative or connective phrase — "the following day", "that same month", "shortly afterwards", "at the time" — you are flagging narrative sequencing, not a checkable fact. Say nothing.
- ESTIMATE TEST: historical quantities are contested and vary by source. NEVER flag a number just because you have a different one in mind. This covers ALL of: death tolls, casualty and crowd/participation estimates, election seat counts, populations, percentages, counts of institutions (churches, factories, farms), growth rates, and the START or END YEAR of a gradual process (a campaign, a wave of closures, a reform period). Flag a quantity ONLY if it is wrong by an order of magnitude or categorically impossible (a percentage over 100; more deaths than people). If the student hedges — "perhaps", "an estimated", "around", "just under", "from X to Y" — that is honest historiography: never flag it, and never flag a figure merely for being "approximate rather than exact". Different figures inside the historians' spread are not errors. (Both the 1921-22 Volga famine ~5m and the 1932-33 famine ~5-7m are real and distinct; never re-date one to the other.) This does NOT cover identifiers that happen to be numbers — an Order number, a Five-Year-Plan number, a Congress number, a specific dated event — those are checkable facts, not estimates.
